...
def find_next(seq: List[int]) -> int:
    ...
    while count < limit and count < len(sequences) - 1:
        ...
        # above = sequences[negative_index - 1][-1]
        below = sequences[negative_index + 1][-1]
        # current_difference = abs(above - below)

        left = sequences[negative_index][-1]
        next_number = left + below
        sequences[negative_index].append(next_number)
        count += 1
