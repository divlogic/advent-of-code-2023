from .utils import Hand, find_answer, get_ranks, get_winnings


def test_hand_strength_five():
    actual = Hand("AAAAA").strength
    expected = 7

    assert actual == expected


def test_hand_strength_four():
    actual = Hand("AA8AA").strength
    expected = 6

    assert actual == expected


def test_hand_strength_full_house():
    actual = Hand("23332").strength
    expected = 5

    assert actual == expected


def test_hand_strength_three_of_a_kind():
    actual = Hand("TTT98").strength
    expected = 4

    assert actual == expected


def test_hand_strength_two_pair():
    actual = Hand("23432").strength
    expected = 3

    assert actual == expected


def test_hand_strength_one_pair():
    actual = Hand("A23A4").strength
    expected = 2

    assert actual == expected


def test_hand_strength_high_card():
    actual = Hand("23456").strength
    expected = 1

    assert actual == expected


def test_comparison_live():
    stronger = Hand("66686")
    weaker = Hand("222A2")
    assert stronger > weaker


def test_comparison_j():
    stronger = Hand("QQQQ2")
    weaker = Hand("JKKK2")
    assert stronger > weaker


def test_comparison_j5():
    stronger = Hand("JKKKK")
    weaker = Hand("QQQQ2")
    assert stronger > weaker


def test_comparison_j5_j_start():
    stronger = Hand("QQQQJ")
    weaker = Hand("JKKKK")
    assert stronger > weaker


def test_comparison_many_j():
    stronger = Hand("JJJJ2")
    weaker = Hand("QQQQ2")
    assert stronger > weaker


def test_comparison_one_pair_with_j_strength():
    pair = Hand("J2345")
    expected = 2
    assert expected == pair.strength


def test_comparison_two_pair_with_j():
    stronger = Hand("22334")
    weaker = Hand("J2345")
    assert stronger > weaker


def test_comparison_full_house_with_j():
    stronger = Hand("23334")
    weaker = Hand("J2345")
    assert stronger > weaker


def test_comparison_three_of_a_kind_with_j():
    stronger = Hand("233J4")
    weaker = Hand("22334")
    assert stronger > weaker


def test_comparison_three_mix_with_j():
    weaker = Hand("233J2")
    stronger = Hand("23332")
    assert stronger > weaker


def test_comparison_three_mix_2_with_j():
    weaker = Hand("233J2")
    stronger = Hand("A33JA")
    assert stronger > weaker


def test_full_house_j():
    hand = Hand("22J33")
    expected = 5
    assert expected == hand.strength


def test_2j_to_three_of_a_kind():
    hand = Hand("JJ234")
    expected = 4
    assert expected == hand.strength


def test_full_house_j_comp():
    stronger = Hand("22J33")
    weaker = Hand("J2233")
    assert stronger > weaker


def test_j_four_of_a_kind():
    hand_1 = Hand("QQQJA")
    hand_2 = Hand("T55J5")
    hand_3 = Hand("KTJJT")
    group = [hand_1, hand_2, hand_3]
    expected = 6
    for hand in group:
        assert expected == hand.strength


def test_get_ranks():
    rank_4 = Hand("QQQJA", 483)
    rank_3 = Hand("T55J5", 684)
    rank_2 = Hand("KK677", 28)
    rank_5 = Hand("KTJJT", 220)
    rank_1 = Hand("32T3K", 765)
    hands = [rank_5, rank_3, rank_4, rank_2, rank_1]

    expected = {1: rank_1, 2: rank_2, 3: rank_3, 4: rank_4, 5: rank_5}
    actual = get_ranks(hands)

    assert expected == actual


def test_get_winnings():
    rank_5 = Hand("KTJJT", 220)
    rank_4 = Hand("QQQJA", 483)
    rank_3 = Hand("T55J5", 684)
    rank_2 = Hand("KK677", 28)
    rank_1 = Hand("32T3K", 765)
    hands = [rank_5, rank_3, rank_4, rank_2, rank_1]

    ranked_hands = get_ranks(hands)
    expected = 5905
    actual = get_winnings(ranked_hands)

    assert expected == actual


def test_find_answer():
    actual = find_answer("input.test.txt")
    expected = 5905

    assert actual == expected
